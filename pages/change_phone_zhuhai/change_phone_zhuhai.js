...
Page({
  ...
  sendCode: function () {
    ...
    if ((/^1[0-9][0-9]\d{8}$/.test(that.data.mobile) && that.data.code_disabled == '')) {
      ...
      //发送验证码
      app.getSmsCode_zhuhai(that.data.mobile, function (res) {
        if (res.ret != 0) {
  ...
  bindCodeInput: function (e) {
    ...
    if (sms_code.length >= 6 && that.data.isRightPhone && that.data.hasSendCode) {
      //校验验证码
      app.checkSmsCode_zhuhai(that.data.mobile, sms_code, function (res) {
        if (res.ret != 0) {//验证失败
  ...
  login: function (e) {
    ...
    if (that.data.button_disabled == '') {//如果按钮激活
      //修改手机号
      app.updateMobile_zhuhai(that.data.mobile, that.data.sms_code, function (res) {
        ...
        wx.reLaunch({
          url: '/pages/received_zhuhai/received_zhuhai',
        })
      });
    } else {
      return;
